...
def load_and_search():
    ...
    ac_first = AC.from_buff(mm, copy=True)  
    ac_second = AC.from_buff(mm, copy=True)  
    ac_third = AC.from_buff(mm, copy=True)  
    ac_four = AC.from_buff(mm, copy=True)  
    print("Matches after loading shared buffer:")
    for id, start, end in ac_first.match(string_to_search):
        print(id, string_to_search[start:end])
    # ... tested mathces in ac_second, third and four as well. Worked.
